...
const TaskCard = ({task,openForm,index}) => {

    const {deleteTask,settaskedit,handledrop,setdragIndex} =useContext(TodoContext)
   ...
   const taskcolor = task.priority==="High"? "text-red-600": task.priority==="Medium"? "text-yellow-600":"text-green-600"
    ...
    return (
        <>
             <div draggable onDragStart={()=>setdragIndex(index)}  onDragOver={(e)=>e.preventDefault()} onDrop={()=>handledrop(index)}
              className="rounded-3xl bg-black w-1/4 p-4 mb-4 border-2 border-emerald-600 text-white">
